...
public class V15_P2_HuluPageParser extends PageParserStrategy {
	...
	protected void parseYear() {
		...
		movieInfo.setYear(nodeHtml.replaceAll("<br.*", "").replaceAll("episod.*", "").trim());
	}
	...
	protected void parseEpisode() {
		Elements node = parentElement.select(".info > div > font ");
		String nodeHtml = node.html();
		movieInfo.setEpisod(nodeHtml.replaceAll(".*br />", "").replaceAll("e.*", "").trim());
	}
}
