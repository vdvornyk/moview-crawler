...
public class V35_1_Site extends AbstractSiteStrategy {
	public static final Logger logger = LoggerFactory.getLogger(V35_1_Site.class);
	...
	private static String ENDPOINT_LINK = "http://vod.shaw.ca/CATEGORY/GENRE/SUB_G/page:PAGE_N/limit:48/#content";
	...
	protected void step3_CollectLinks() {
		...
		for (Element link : genres) {
			...
			if (splited.length == 4) {
				String linkToSubGenre = SITE_URL + "/" + splited[1] + "/" + splited[2] + "/" + splited[3] + "/limit:48/";
				...
				try {
					...
					if (pages.size() != 0) {
						...
						for (int i = 1; i <= max; i++) {
							String pageUrl = ENDPOINT_LINK.replace("CATEGORY", splited[1]).replace("GENRE", splited[2]).replace("SUB_G", splited[3]).replace("PAGE_N", "" + i);
							logger.info("processing genre {} pageEndpoint {}", splited[2], pageUrl);
							PageWithMovies pageWithMovies = new PageWithMovies(splited[2], splited[3], siteName, pageUrl);
							fullPageWithMoviesList.add(pageWithMovies);
						}

					} else {
						logger.info("processing genre {} pageEndpoint {}", splited[2], linkToSubGenre);
						PageWithMovies pageWithMovies = new PageWithMovies(splited[2], splited[3], siteName, linkToSubGenre);
						fullPageWithMoviesList.add(pageWithMovies);
					}
