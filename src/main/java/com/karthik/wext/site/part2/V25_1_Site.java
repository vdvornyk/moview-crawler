...
public class V25_1_Site extends AbstractSiteStrategy {
	...
	protected void step3_CollectLinks() {
		...
		for (int i = 0; i < genres.size(); i++) {
			...
			try {
				String genreOnClick = genreLink.attr("onclick");
				LogUtils.logger.info(genreOnClick);
				String genreHref = urlTemplate + GNR + genreOnClick.replaceAll(".*category=", "").replaceAll("'.*", "");
